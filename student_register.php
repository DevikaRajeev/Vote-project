...
{
$query="insert into user_register(firstname,secondname,email,dob,address,gender,dept,phone_no,password,reenter_password,status,astatus)values('$user_fname','$user_lname','$user_email','$user_dob','$user_address','$user_sex','$dept','$user_phno','$user_password','$user_cpassword','','Pending')";
		...
		if($r == 1)
		{
$query="insert into login(username,password,usertype,status)values('$user_email','$user_password','student','Pending')";
...
// echo $query;
 mysql_query($query,$connect);
		...
		if($r==2 )
		{
				...
				echo "<script>alert('Registerd successfully!! Must Login..');document.location.href='login.php';</script>";
...
function moblen()
{
tot=document.getElementById('pno').value;
	t=tot.length;
	if(t>10)
	{
		alert("Phone number should not greater than 10");
	...
	{
		alert("Phone number should not lessthan 10");
		document.getElementById('pno').value="";
		document.RegisterForm.name.focus();
		return false;
	}
...
<h3>
          <center>
    <b style="font-family:Georgia">Student Registration Form</b>
  </center><br>
        </h3>
  ...
  <div class="col-sm-9"> 
    ...
    <option>IIIrd BA HINDI</option></select>
    </div>
									</div>
  ...
  <div class="col-sm-9"> 
    <input type="text" id="pno"  name="pno" maxlength="10" onchange='moblen(this.value)' class="col-xs-10 col-sm-5" style="border: 1px solid #D5D5D5;padding: 5px 4px 6px;" />
  </div>
									</div>
